...
| 단계 | 기간  | 구분            | 주요 작업                                                    | 산출물                       |
| -: | --- | ------------- | -------------------------------------------------------- | ------------------------- |
|  1 | 3주차 | 준비            | EKS 검증용 branch 생성 및 Serverless 버전과 분리 기준 정리              | eks branch                |
|  2 | 3주차 | 기획            | EKS 버전의 목표, 검증 범위, 비용 관리 기준 정의                           | project-plan.md           |
|  3 | 3주차 | Backend       | Lambda 기반 backend와 별도로 Express HTTP server entrypoint 추가 | `src/server`              |
|  4 | 3주차 | Backend       | Local HTTP server 실행 및 health check 확인                   | `/health` 응답              |
|  5 | 3주차 | Container     | Backend container image 생성을 위한 Dockerfile 작성             | backend.Dockerfile        |
|  6 | 3주차 | Container     | Local Docker 환경에서 backend container 실행 검증                | Docker container          |
|  7 | 3주차 | Registry      | EKS 배포용 container image 저장소 구성                           | ECR Repository            |
|  8 | 3주차 | CI/CD         | GitHub Actions 기반 image build / push workflow 구성         | eks-image-build.yml       |
|  9 | 3주차 | Security      | Container image 취약점 점검 단계 추가                             | Trivy scan log            |
| 10 | 3주차 | Infra         | EKS Terraform 구성의 기본 디렉터리 및 module 구조 작성                 | EKS env/module            |
| 11 | 3주차 | Infra         | VPC, EKS Cluster, NodeGroup 구성 및 배포 검증                   | EKS Cluster               |
| 12 | 3주차 | Ingress       | AWS Load Balancer Controller 설치 및 동작 확인                  | Controller Pod            |
| 13 | 3주차 | IAM           | ALB Controller용 IRSA 구성 및 권한 분리 검증                       | IAM Role / ServiceAccount |
| 14 | 3주차 | IAM           | Backend Pod용 IRSA 구성 및 AWS 서비스 접근 권한 분리                  | Backend IAM Role          |
| 15 | 3주차 | Kubernetes    | Namespace, ServiceAccount, Deployment manifest 작성        | K8s manifest              |
| 16 | 3주차 | Kubernetes    | Service, Ingress manifest 작성 및 ALB 연결 구조 구성              | ALB Ingress               |
| 17 | 3주차 | 검증            | Backend Pod 배포 후 Kubernetes 내부 health check 확인           | `/health` success         |
| 18 | 3주차 | 검증            | ALB Ingress를 통한 외부 API 호출 검증                             | API success               |
| 19 | 3주차 | GitOps        | Argo CD 설치 및 GitOps 검증 환경 구성                             | Argo CD Pods              |
| 20 | 3주차 | GitOps        | Argo CD Application 구성 및 manifest sync 상태 확인             | Synced / Healthy          |
| 21 | 3주차 | Observability | Prometheus / Grafana 기반 모니터링 스택 구성                       | Monitoring Stack          |
| 22 | 3주차 | Observability | Prometheus Targets 및 Grafana 기본 metric 확인                | Dashboard capture         |
| 23 | 3주차 | 문서화           | EKS 구축 및 검증 결과 캡처 정리                                     | `evidence.md`             |
| 24 | 3주차 | 문서화           | 진행 중 발생한 오류와 해결 과정을 문서화                                  | `troubleshooting.md`      |
| 25 | 3주차 | 문서화           | 장애 대응 및 운영 확인 절차 정리                                      | `runbook.md`              |
| 26 | 3주차 | 문서화           | EKS 아키텍처 설계 의도와 구성 요소 정리                                 | `eks-design.md`           |
| 27 | 3주차 | 비용 관리         | 검증 완료 후 NodeGroup 축소 또는 Terraform destroy 수행             | Cost control              |
